...
MAX_ORDER_NOTIONAL = 10_000  # sanity ceiling only — full-roll compounding per user 2026-08-20
MAX_DAILY_LOSS_PCT = 0.10    # kill switch: no new buys once down 10% of the day-start equity
...
def _roll_day_and_check_kill_switch(state: dict) -> None:
    ...
    limit = state["day_start_equity"] * MAX_DAILY_LOSS_PCT
    if loss >= limit:
        raise SystemExit(f"KILL SWITCH: down ${loss:,.2f} today (limit ${limit:,.2f}). No more orders.")
